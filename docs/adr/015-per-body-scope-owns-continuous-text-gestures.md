...
Under mobile **selection policy**, the viewport **gates** message-then-text: on pointer-down, if the hit would route to text (already **message-selected** body text while text is inactive, or the live **text selection subject**), it does not register its long-press for that pointer. The scope’s content-gated touch long-press wins, stays live for drag-extend, and shows chrome. The facade **adopts** the **text selection subject** from the first non-collapsed markdown range. While several messages are message-selected and text is inactive, every selected mounted body may expose a surface; while text is live, only the subject mounts a selection surface (sibling `putDocument` would re-expand the shared registry and break Select All / one-message ranges). Retarget onto another selected body uses reported body paint bounds and facade `enterTextSelection` after the viewport keeps the long-press. Touch consecutive-tap word/block entry stays off; mouse multi-click remains desktop. Programmatic `enterTextSelection` stays for hosts/tests and is not the mobile gesture hot path. Single tap still dismisses text and keeps membership (viewport idle-tap path).

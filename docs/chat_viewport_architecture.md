...
**Inline-разделитель.** Первое сообщение дня строится как `DatedMessage` —
кастомный render object: разделитель сверху, тело снизу (оба в своих
`RepaintBoundary`). `RenderChatScrollView` считает `startsDay` / `dayBucket` в
`_buildMessage` и кладёт в `ParentData` — per-frame обход это чистое чтение
полей.
...
перестраиваться). Закреплён у верхней кромки (`topPadding`), не двигается.

**Угасание у кромки.** Расставляя детей (`_setOffset`), viewport считает для
разделителя верхней секции `dividerOpacity` по его Y: полная, как только
разделитель опускается ниже хедера; затухает до 0 за короткую полосу
(`_kDividerFadeBand`) по мере подъёма в его зону.
`DatedMessage` красит разделитель с этой opacity через `pushOpacity`, сохраняя
его высоту — место остаётся зарезервированным. Поэтому inline-разделитель и
плавающий хедер никогда не видны одновременно: подъезжая к верху, разделитель
плавно угасает, дату держит хедер. `DatedMessage` — не `RepaintBoundary` (его
перекрашивают каждый кадр ради opacity), но оба ребёнка — `RepaintBoundary`,
поэтому кадр лишь композитит два слоя.
